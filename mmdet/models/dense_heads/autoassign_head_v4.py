...
class AutoAssignHeadV4(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.mu = nn.Parameter(torch.tensor([[0.0,0.0]]), requires_grad=False)
        self.sigma = nn.Parameter(torch.tensor([[2.0, 2.0]]), requires_grad=False)
        # self.mu = nn.Parameter(torch.rand(1,2))
        # self.sigma = nn.Parameter(torch.rand(1,2))
        print("*"*48)
    ...
    def cam(self, feats):
        flatten_feats = feats[-1]#[ feat.permute(0, 2, 3, 1).reshape(-1, feat.size(1)) for feat in feats ]
        import pdb; pdb.set_trace()
    ...
    def loss(self,
        ...
        neg_prob = flatten_cls_scores[neg_point_inds].sigmoid() * flatten_centerness[neg_point_inds].sigmoid()[:,None]
    ...
    def _get_single_pos_loss(self,
        ...
        if num_pos > 0 :
            ...
            # import pdb;pdb.set_trace()
            G_d = self.centerness_target(pos_bbox_targets)
            C_p = torch.exp(pos_prob.detach() * 3.0)
            
            w_plus = (C_p * G_d)/(C_p * G_d).sum()
            ...
            # pos_loss = 0.25 * (1-prob_sum)**2 * F.binary_cross_entropy(prob_sum,torch.ones_like(prob_sum)
    ...
    def centerness_target(self, pos_bbox_targets):
        ...
        bottem = pos_bbox_targets[:, 3]
        
        offset_x = left - (left +right) / 2
        offset_y = top - (top + bottem) /2
        offset = torch.cat([offset_x.view(-1,1),offset_y.view(-1,1)],dim=1)
        # import pdb;pdb.set_trace() 
        G = torch.exp((-(offset - self.mu)**2/(2*self.sigma  ** 2)).sum(1))
        # centerness_targets = (
        #     left_right.min(dim=-1)[0] / left_right.max(dim=-1)[0]) * (
        #         top_bottom.min(dim=-1)[0] / top_bottom.max(dim=-1)[0])
        # print(self.mu, self.sigma)

        # sigmod = self.sigma[0,0]*self.sigma[0,1]
        # return G/(2*np.pi) # use G/(2*np.pi/sigmod) or remove 2*np.pi will lead to mAP=0
        return G # use G/(2*np.pi/sigmod) lead to mAP=0
